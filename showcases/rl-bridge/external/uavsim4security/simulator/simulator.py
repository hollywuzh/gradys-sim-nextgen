...
from security.attack_manager import AttackManager
...
from results.dataset_exporter import DatasetExporter
...
class Simulator:
    ...
    def __init__(self,
        ...
        self.dataset_exporter = DatasetExporter(self)
        self.attack_manager = AttackManager(self)
        self.security_attack_events = []
        self.transmission_observers = []
        ...
        self.attack_manager.initialize()
    ...
    def register_transmission_observer(self, observer):
        self.transmission_observers.append(observer)

    def notify_unicast_transmission(self, packet, src_drone_id, dst_drone_id):
        for observer in list(self.transmission_observers):
            observer(packet, src_drone_id, dst_drone_id, self.env.now)
    ...
    def show_performance(self):
        ...
        export_summary = self.dataset_exporter.finalize()
        if export_summary is not None:
            print('Dataset exporter wrote: ', export_summary["num_records"], ' routing transitions')
            print('Dataset JSONL path: ', export_summary["jsonl_path"])
