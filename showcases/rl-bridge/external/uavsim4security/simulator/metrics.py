...
class Metrics:
    ...
    def record_security_event(self, event_name, increment=1):
        self.security_event_counts[event_name] += increment
    ...
    def print_metrics(self):
        ...
        e2e_delay = self._safe_mean(self.deliver_time_dict.values()) / 1e3

        # calculate the packet delivery ratio
        pdr = len(self.datapacket_arrived) / self.datapacket_generated_num * 100  # in %

        # calculate the throughput
        throughput = self._safe_mean(self.throughput_dict.values()) / 1e3

        # calculate the hop count
        hop_cnt = self._safe_mean(self.hop_cnt_dict.values())

        # calculate the routing load
        rl = self.control_packet_num / len(self.datapacket_arrived) if self.datapacket_arrived else float('nan')

        # channel access delay
        average_mac_delay = self._safe_mean(self.mac_delay)
        ...
        for event_name in sorted(self.security_event_counts):
            print(f'Security metric [{event_name}] is: ', self.security_event_counts[event_name])
