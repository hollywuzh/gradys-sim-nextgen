...
class Grad:
    ...
    def packet_reception(self, packet, src_drone_id):
        ...
        if isinstance(packet, GradMessage):
            ...
            if packet_copy.attached_data_packet is not None:
                packet_copy.attached_data_packet = copy.copy(packet.attached_data_packet)
                        ...
                        if packet_copy.remaining_value > 0:
                            ...
                            if self.cost_table.has_entry(data_packet.dst_drone.identifier):
                                ...
                                if est_cost <= packet_copy.remaining_value:
                                    ...
                                    packet_copy.attached_data_packet.increase_ttl()
                                    self.my_drone.transmitting_queue.put(packet_copy)
                            else:
                                pass
                        else:
                ...
                if self.my_drone.identifier == packet_copy.target.identifier:
                    ...
                    for item in self.my_drone.waiting_list:
                        ...
                        if dst_drone.identifier == packet_copy.originator.identifier:
                            recovered_packets.append(item)
                        else:
                            remaining_waiting_list.append(item)

                    self.my_drone.waiting_list = remaining_waiting_list

                    for item in recovered_packets:
