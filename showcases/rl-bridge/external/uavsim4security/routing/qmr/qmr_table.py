...
class QMRTable(BaseTable):
    ...
    def build_serializable_routing_snapshot(self, packet, destination, chosen_neighbor_id=None):
        cur_time = self.env.now
        candidate_neighbors, sub_candidate_neighbors, actual_velocity_dict, required_velocity = (
            self.filter_space_of_exploration(packet, destination, cur_time)
        )

        candidate_ids = {item[0] for item in candidate_neighbors}
        sub_candidate_ids = {item[0] for item in sub_candidate_neighbors}
        candidate_k = {item[0]: item[1] for item in candidate_neighbors}
        sub_candidate_velocity = {item[0]: item[1] for item in sub_candidate_neighbors}

        all_neighbors = []
        for neighbor_id, entry in self.table.items():
            snapshot = {
                "neighbor_id": int(neighbor_id),
                "recorded_pos": self._to_float_list(entry.get("recorded_pos")),
                "recorded_vel": self._to_float_list(entry.get("recorded_vel")),
                "remain_energy": self._safe_float(entry.get("remain_energy")),
                "q_value": self._safe_float(entry.get("q_value")),
                "weighted_q_value": self._safe_float(entry.get("weighted_q_value")),
                "lq": self._safe_float(entry.get("lq")),
                "k_factor": self._safe_float(candidate_k.get(neighbor_id, entry.get("k_factor"))),
                "actual_velocity": self._safe_float(actual_velocity_dict.get(neighbor_id, entry.get("actual_velocity"))),
                "delay_us": self._safe_float(entry.get("delay")),
                "updated_time_us": self._safe_float(entry.get("updated_time")),
                "is_candidate": neighbor_id in candidate_ids,
                "is_sub_candidate": neighbor_id in sub_candidate_ids,
                "sub_candidate_velocity": self._safe_float(sub_candidate_velocity.get(neighbor_id)),
            }
            if "trust_score" in entry:
                snapshot["trust_score"] = self._safe_float(entry.get("trust_score"))
                snapshot["trust_success_count"] = int(entry.get("trust_success_count", 0))
                snapshot["trust_failure_count"] = int(entry.get("trust_failure_count", 0))

            all_neighbors.append(snapshot)

        chosen_snapshot = None
        if chosen_neighbor_id is not None and chosen_neighbor_id in self.table:
            chosen_snapshot = next(
                (item for item in all_neighbors if item["neighbor_id"] == int(chosen_neighbor_id)),
                None,
            )

        return {
            "required_velocity": self._safe_float(required_velocity),
            "neighbor_count": len(self.table),
            "candidate_neighbors": [int(item[0]) for item in candidate_neighbors],
            "sub_candidate_neighbors": [int(item[0]) for item in sub_candidate_neighbors],
            "neighbors": all_neighbors,
            "chosen_neighbor_snapshot": chosen_snapshot,
        }
    ...
    def get_last_max_q_value_of_neighbor(self, neighbor_id):
        if neighbor_id not in self.table:
            return None

        last_max_q = self.table[neighbor_id].get("last_max_q")
        if last_max_q is None:
            return self.table[neighbor_id].get("q_value")
        return last_max_q
