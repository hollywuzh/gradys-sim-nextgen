...
def sinr_calculator(my_drone, main_drones_list, all_transmitting_drones_list):
    ...
    for pair in main_drones_list:  # each pair includes the main drone id and the channel id
        ...
        external_interferers = simulator.attack_manager.get_external_interferers(receiver, channel_id)
        for interferer in external_interferers:
            interference_link_path_loss = general_path_loss(receiver, interferer)
            interference_power += interferer.transmit_power * interference_link_path_loss
            real_interference_nodes.append(interferer.identifier)
