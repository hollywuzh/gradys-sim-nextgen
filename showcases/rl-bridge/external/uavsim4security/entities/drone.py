...
from routing.sqmr.sqmr import SQMR
...
class Drone:
    ...
    def _create_routing_protocol(self):
        protocol_name = config.ROUTING_PROTOCOL.upper()

        if protocol_name == 'DSDV':
            return Dsdv(self.simulator, self)

        if protocol_name == 'GRAD':
            return Grad(self.simulator, self)

        if protocol_name == 'GREEDY':
            return Greedy(self.simulator, self)

        if protocol_name == 'OPAR':
            return Opar(self.simulator, self)

        if protocol_name == 'QGEO':
            return QGeo(self.simulator, self)

        if protocol_name == 'QROUTING':
            return QRouting(self.simulator, self)

        if protocol_name == 'QFANET':
            return QFanet(self.simulator, self)

        if protocol_name == 'QMR':
            return QMR(self.simulator, self)

        if protocol_name == 'SQMR':
            return SQMR(self.simulator, self)

        raise ValueError(f'Unsupported routing protocol: {protocol_name}')
    ...
    def generate_data_packet(self, traffic_pattern='Poisson'):
        ...
        while True:
            if not self.sleep:
                ...
                self.simulator.dataset_exporter.record_packet_generated(pkd)
                    ...
                    if not self.transmitting_queue.empty():
                        ...
                        if self.env.now < packet.creation_time + packet.deadline:  # this packet has not expired
                            if isinstance(packet, DataPacket):
                                if self.simulator.attack_manager.should_drop_forwarded_packet(self, packet):
                                    continue
                ...
                if flag:
                    ...
                    if max_sinr >= config.SNR_THRESHOLD:
                        ...
                        if pkd.get_current_ttl() < config.MAX_TTL:
                            ...
                            if self.simulator.attack_manager.should_drop_received_packet(self, pkd, sender):
