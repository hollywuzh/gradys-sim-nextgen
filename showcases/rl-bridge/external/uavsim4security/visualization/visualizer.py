...
class SimulationVisualizer:
    ...
    def _draw_visualization_frame(self, fig, current_time):
        ...
        attacker_snapshot = self._get_network_attack_snapshot(current_time)
        if attacker_snapshot:
            for ax in [ax_data, ax_ack]:
                self._draw_network_attackers(ax, drone_positions, attacker_snapshot)

        jammer_snapshot = self._get_phy_jammer_snapshot(current_time)
        if jammer_snapshot is not None:
            for ax in [ax_data, ax_ack]:
                self._draw_phy_jammer(ax, jammer_snapshot)
        ...
        for attack_name, marker in [("BLACKHOLE", "o"), ("GRAYHOLE", "s")]:
            if attack_name in attacker_snapshot:
                data_legend.append(
                    Line2D([0], [0], marker=marker, color=self.attack_colors[attack_name], linestyle='None',
                           markersize=10, markerfacecolor='none', markeredgewidth=2, label=f"{attack_name} Node")
                )
        if jammer_snapshot is not None:
            jammer_label = "PHY Jammer (Active)" if jammer_snapshot["active"] else "PHY Jammer (Idle)"
            data_legend.append(
                Line2D([0], [0], marker='X', color=jammer_snapshot["color"], linestyle='None',
                       markersize=10, label=jammer_label)
            )
    ...
    def create_animations(self):
        ...
        try:
            ...
            # Save the animation
            animation_file = os.path.join(self.output_dir, f"{config.VIS_OUTPUT_BASENAME}.gif")
            if animation_frames:
    ...
    def finalize(self):
        ...
        # Create interactive visualization only when enabled
        if config.ENABLE_INTERACTIVE_VISUALIZATION:
    ...
    def create_interactive_visualization(self):
        ...
        def update_plot(current_time):
            ...
            attacker_snapshot = self._get_network_attack_snapshot(current_time)
            if attacker_snapshot:
                self._draw_network_attackers(ax_data, drone_positions, attacker_snapshot)
                self._draw_network_attackers(ax_ack, drone_positions, attacker_snapshot)

            jammer_snapshot = self._get_phy_jammer_snapshot(current_time)
            if jammer_snapshot is not None:
                self._draw_phy_jammer(ax_data, jammer_snapshot)
                self._draw_phy_jammer(ax_ack, jammer_snapshot)
    ...
    def _get_network_attack_snapshot(self, current_time):
        """Return network-layer attacker metadata and recent drop events."""
        attack_manager = getattr(self.simulator, "attack_manager", None)
        if attack_manager is None:
            return {}

        current_time_us = current_time * 1e6
        active = attack_manager.is_attack_window_active()
        snapshot = {}

        for attack_name in ("BLACKHOLE", "GRAYHOLE"):
            attacker_ids = sorted(attack_manager.get_attacker_ids_for(attack_name))
            if not attacker_ids:
                continue

            recent_events = [
                event for event in getattr(self.simulator, "security_attack_events", [])
                if event["attack_name"] == attack_name and current_time_us - self.vis_frame_interval <= event["time_us"] <= current_time_us
            ]
            event_map = {event["drone_id"]: event for event in recent_events}

            snapshot[attack_name] = {
                "attacker_ids": attacker_ids,
                "recent_events": event_map,
                "active": active,
            }

        return snapshot

    def _draw_network_attackers(self, ax, drone_positions, attacker_snapshot):
        """Highlight network-layer attacker nodes and recent packet drops."""
        marker_map = {
            "BLACKHOLE": "o",
            "GRAYHOLE": "s",
        }
        ...
        for attack_name, meta in attacker_snapshot.items():
            color = self.attack_colors[attack_name]
            marker = marker_map[attack_name]
            label_prefix = label_map[attack_name]

            for drone_id in meta["attacker_ids"]:
                if drone_id not in drone_positions:
                    continue

                x0, y0, z0 = drone_positions[drone_id]
                ax.scatter(
                    x0,
                    y0,
                    z0,
                ...
                recent_event = meta["recent_events"].get(drone_id)
    ...
    def _get_phy_jammer_snapshot(self, current_time):
        """Return PHY jammer metadata for visualization at the given time."""
        attack_manager = getattr(self.simulator, "attack_manager", None)
        if attack_manager is None:
            return None

        jammer_attack = next(
            (attack for attack in getattr(attack_manager, "attacks", []) if getattr(attack, "name", "") == "PHY_JAMMING"),
            None,
        )
        if jammer_attack is None or getattr(jammer_attack, "jammer", None) is None:
            return None

        current_time_us = current_time * 1e6
        active = jammer_attack.start_time_us <= current_time_us <= jammer_attack.end_time_us
        color = "red" if active else "gray"

        return {
            "coords": jammer_attack.jammer.coords,
            "radius": jammer_attack.jammer.radius,
            "active": active,
            "color": color,
        }

    def _draw_phy_jammer(self, ax, jammer_snapshot):
        """Draw jammer location and impact sphere."""
        x0, y0, z0 = jammer_snapshot["coords"]
        radius = jammer_snapshot["radius"]
        color = jammer_snapshot["color"]
        alpha = 0.18 if jammer_snapshot["active"] else 0.08

        ax.scatter(x0, y0, z0, color=color, s=220, marker='X', edgecolors='black', zorder=95)
        ax.text(
            x0,
            y0,
            z0,
        ...
        xs = x0 + radius * np.outer(np.cos(u), np.sin(v))
        ys = y0 + radius * np.outer(np.sin(u), np.sin(v))
        zs = z0 + radius * np.outer(np.ones(np.size(u)), np.cos(v))
        ax.plot_wireframe(xs, ys, zs, rstride=2, cstride=2, color=color, linewidth=0.5, alpha=alpha)
